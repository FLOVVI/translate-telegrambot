...
def start(message):
    ...
    if first:
        ...
        for i in records:
            l.lang = i[0]
        if l.lang != "zh-cn":
            result_lang = LANGUAGES.get(l.lang)
...
def handle_photo(message):
    ...
    for i in records:
        l.lang = (i[0])
    translator = Translator()
    text_rec = text_recognition('translate.jpg')
    translate_result = translator.translate(text_rec, dest=l.lang).text
    bot.send_message(message.chat.id, f"Распознанный текст:\n\n{text_rec}\n\nПеревод:\n\n[{l.lang}] {translate_result}")
    try:
...
def handle_text(message):
    ...
    for i in records:
        l.spelling_TF = (i[0])
    result = transl.translate(message.text, dest=l.lang)
    if l.spelling_TF:
        try:
            spell = YandexSpeller()
            if str(spell.spelled(message.text)) != str(message.text):
                ...
                if result.src == l.lang and l.lang == "ru":
                    result = transl.translate(spell.spelled(message.text), dest="en")
                    bot.send_message(message.chat.id, f'[en] {result.text}')
                elif result.src == l.lang:
                    result = transl.translate(spell.spelled(message.text), dest="ru")
                    bot.send_message(message.chat.id, f'[ru] {result.text}')
                else:
                    result = transl.translate(spell.spelled(message.text), dest=l.lang)
                    bot.send_message(message.chat.id, f'[{l.lang}] {result.text}')
            else:
                if result.src == l.lang and l.lang == "ru":
                    result = transl.translate(message.text, dest="en")
                    bot.send_message(message.chat.id, f'[en] {result.text}')
                elif result.src == l.lang:
                    result = transl.translate(message.text, dest="ru")
                    bot.send_message(message.chat.id, f'[ru] {result.text}')
                else:
                    result = transl.translate(message.text, dest=l.lang)
                    bot.send_message(message.chat.id, f'[{l.lang}] {result.text}')
